<template>
    <div class="offer" id="print">
        <div class="offer-title">广告扇实时报价</div>
            ...
            <div class="content">
                <div class="content-left">
                    ...
                    <div class="content-left-bottom">
                        ...
                        <div class="list" v-if="production.IsShowPage">
                            <span class="title">P 数: </span>
                            <div class="item">
                                <div class="choose-list" v-for="(item,i) in production.Pages" :key="i" @click="setProduct('Pages', i)">
                                    ...
                                    <img v-else :class="{'choose-img': true, active : item.IsDefault}" :src="$store.state.port.imgBaseUrl + item.ImageUrl" alt="">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="list" v-if="production.Crafts.length">
                        ...
                        <div class="list">
                            ...
                            <div class="item">
                                <div class="choose-list" v-for="(item,i) in production.Quantities" :key="i" @click="setProduct('Quantities', i)">
                                    <span v-if="item.DisplayMode == 0" :class="{'choose-item': true, active : item.IsDefault}">{{item.Number + unit}}</span>
                                    <img v-else :class="{'choose-img': true, active : item.IsDefault}" :src="$store.state.port.imgBaseUrl + item.ImageUrl" alt="">
                                </div>
                            </div>
                        </div>
                        <div class="list" v-if="production.Urgent">
                            <span class="title">加急: </span>
                            <div class="item">
                                <div class="choose-list">
                                    <span :class="{'choose-item' : true, active : urgent}" @click="handleChooseUrgent">我要加急</span>
                                </div>
                            </div>
                        </div>
                        <div class="list" v-if="couponList.length">
                            <span class="title coupon">优惠券: </span>
                            <div class="item" style="width: 507px;">
                                <div class="choose-list" v-for="(item,i) in couponList" :key="i">
                                    <div style="display:flex;color: #fff;margin-bottom: 10px;margin-right: 10px;cursor: pointer;">
										<div :class="couponIndex == i && (total >= item.LimitPrice) ? 'choose-item-active' : ''" :style="`width: 140px;height: 55px;text-align:center;background:${total <= item.LimitPrice ? '#999999' : '#199ED8'};position: relative; overflow: hidden;`" @click="handleCoupon(i)">
											<p>¥&nbsp;&nbsp;<span style="font-size: 28px;font-weight: 800;">{{item.Amount}}</span></p>
											<p>满{{item.LimitPrice}}元可用</p>
										</div>
										<div v-show="item.IsReceived != 1" @click.stop="handleReceive(item)" style="display: inline-block;width: 70px;height: 55px;line-height: 55px;text-align:center;background:#FF9900;">领取</div>
									</div>
                                </div>
                            </div>
                        </div>
                        <div class="list">
                            <span class="title file">文件: </span>
                            <div :class="{'way-btn':true, 'have-file': have}" @click="handleClick('have')">
                                <img src="/img/print/xqy_scwj_icon.png" alt="">
                                <span>已有文件</span>
                            </div>
                            <div :class="{'way-btn':true, 'no-file': no}" @click="handleClick('no')">
                    ...
                    <div class="template-list" v-if="list.length">
                        <div :class="['template-item',tempIndex == i ? 'active' : '']" v-for="(item,i) in list" :key="i" @click="handleChooseTemp(i)">
                            <div class="template-block">
                                <div v-if="item.FacePicture" class="item-image" :style="{'background-image': `url(${$store.state.port.imgBaseUrl+ item.FacePicture})`}"></div>
                            </div>
                ...
                <div class="content-right">
                    <div class="choose-file" v-if="have">
                        <el-upload
                            ...
                            <div class="upload">
                                ...
                                <span v-else class="success-file">{{printprice.FileName}}<i class="el-icon-close" @click.stop="handleDel"></i></span>
                            </div>
                            <div slot="tip" class="el-upload__tip">支持pdf、jpg、zip、rar、cdr、ai、psd格式</div>
                        </el-upload>
                    </div>
                    <div class="choose-file self" v-if="no">
                    ...
                    <div class="price-wrap">
                        <p class="amount">
                            ...
                            <span v-if="couponIndex != null" style="color:red;font-size: 14px;">(已优惠{{specialrate}}元)</span>
                        </p>
                        <p class="time">
                            <img src="/img/print/xqy_time_icon.png" alt="">
                            <span v-if="currentDate < xiadanDate">今天<span style="color:#0083E9;">{{production.PressTime}}</span>前下单，预计<span>{{time}}</span>可以发货</span>
                            <span v-else>现在下单，预计<span>{{time}}</span>可以发货</span>
                        </p>
                    </div>
                    <div class="order btn" @click="handleToCar(true)">立即下单</div>
                    <div class="add-cart btn" @click="handleToCar(false)">加入购物车</div>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        handleToCreate() {
            ...
            let num = this.production.ProductType.FromClass
            let subtitle = this.production.ProductType.TypeName
            let id = this.production.TypeId
            this.$router.push({path: '/templateList/templateCenter', query: {id: id, n: num, title: '', subtitle: subtitle}})
        },
        handleSure() {  // 确定选择
            this.dialogVisible = false
        ...
        //     this.tempList = []
        //     if(val > 1) {
        //         this.tempList = this.list.slice(this.page.pageSize * (val - 1), this.page.pageSize * val - 1)
        //     }else{
        //         this.tempList = this.list.slice(0, 10)
        //     }
        // },
        ...
        getPrintSize() {
            new Promise((resolve, reject) => {
                this.$axios.get('/ProductSheet?TypeID=' + 38).then(res => {
